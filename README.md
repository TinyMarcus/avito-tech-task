...
│   ├── models                              // основные структуры и DTO
│   ├── repository                          // методы для взаимодействия с БД
│   └── utils                               // утилиты
├── cmd/dynamic-user-segmentation-service   // точка входа в приложение
└── db                                      // SQL файлы с миграциями (НЕ ИСПОЛЬЗУЮТСЯ)
```
...
* подхода разделения севриса на разные слои (чистая архитектура);
* работы с СУБД PostgreSQL с использованием библиотеки `sqlx` и написанием сырых SQL-запросов для более высокой производительности при высоких нагрузках;
